...
concreteMeasurements= unit_price_quantities.UnitPriceQuantities(site.getUnitPrice('concrete'))
concreteMeasurements.appendMeasurement(textComment='test A', nUnits= 1, length= 1.0, width=None, height=None)
ch01.appendUnitPriceQuantities(concreteMeasurements)
...
elementary_quantities_ref_list= [['PEON', 'Specialized laborer', 9.5, 'h', 130.625, '€'], ['rawConcrete', 'Raw concrete', 1.0, 'm3', 75.0, '€'], ['rawSteel', 'Reinforcing steel', 60.0, 'kg', 72.0, '€'], ['OFIC1a', '1st class tradesman', 0.95, 'h', 17.233, '€'], ['OFIC2a', '2nd class tradesman', 0.125, 'h', 2.155, '€']]

price= site.getPrice()
testOK= (abs(price-297.34)/297.34<1e-6)

for row, ref_row in zip(elementary_quantities_list, elementary_quantities_ref_list):
    code= row[0]
    ref_code= ref_row[0]
